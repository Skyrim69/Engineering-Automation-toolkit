...
def Insert_end_of_Unit(basefile, content_to_insert):
    ...
    if target_table:
        # insert each element properly
        for element in content_to_insert.contents:
            target_table.insert_before(element)

        print("Inserted before 'End of Test Unit:'")

    return basefile

def insert_end_of_Group_fixture(basefile, content_to_insert, group_name):

    #find table for that <td>
    end_unit_cell= basefile.find("td", string=lambda f:f and "End of Test Group:" in f and group_name in f)
    if not end_unit_cell:
...
for file, soup in file_soups[1:]:
    ...
    if CheckTestGroup(Duplicate_file,soup):
        ...
        group_text = soup.find(string=lambda f: f and "Test Group:" in f)
        if group_text:
        ...
        for fixture in fixtures:
            ...
            fixture_group = Duplicate_file.find("a", string=lambda f: f and f.strip() == f"Test Fixture: {fixture_name}")
            #group missing Test fixture
            if fixture_group:
                print(f"Fixture {fixture_name} already found in base → skipping")
                continue
            if not fixture_group:
                start_fix_table = fixture.find_parent("table")
                current_fix = start_fix_table
                collect_fix_data =[]
                print(f"Processing group: {fixture_name}")
                while current_fix:
                    collect_fix_data.append(copy.deepcopy(current_fix))
                    text = current_fix.get_text(" ",strip=True)
                    fresh_text = text.strip()
                    if "End of Test Group:" in fresh_text and fixture_name in fresh_text:
                        print("Same Test fixture END found → stopping")
                        break
                    current_fix = current_fix.find_next_sibling()
                    if not current_fix:
                        break
                    if current_fix.name == "table":
                        text = current_fix.get_text(" ", strip=True)
                    if "Test Fixture:" in text and fixture_name not in text:
                        print("Next fixture found → stopping")
                        break
                insert_end_of_Group_fixture(Duplicate_file, collect_fix_data ,group_name)
